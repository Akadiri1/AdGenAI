...
export function FinalVideoPanel({ adId }: { adId: string }) {
  ...
  const clips = scenes.map(s => ({ 
    videoUrl: (s.finalClipUrl ?? s.videoClipUrl)!, 
    audioUrl: s.voiceoverUrl,
    scene: s.sceneNumber 
  })).filter(c => c.videoUrl);
  
  const currentClip = clips[currentIdx]?.videoUrl;
  const currentAudio = clips[currentIdx]?.audioUrl;
    ...
    <div className="mb-6 rounded-3xl border-2 border-success/20 bg-gradient-to-br from-success/5 via-white to-accent/5 p-5 shadow-sm">
      ...
      {isReady && clips.length > 0 ? (
        <div className="space-y-3">
          ...
          <VideoStitcher clips={clips as any} adId={adId} />
          ...
          {clips.length > 1 && (
            <details className="rounded-xl border border-black/5 px-3 py-2 text-xs">
              <summary className="cursor-pointer font-semibold text-text-secondary hover:text-text-primary">
                Download scenes separately
              </summary>
              <div className="mt-2 flex flex-wrap gap-2">
                {clips.map((c, i) => (
                  <a key={i} href={c.videoUrl} download
                    className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg border border-black/10 bg-white px-3 text-xs font-semibold text-text-primary hover:bg-bg-secondary">
                    <Download className="h-3.5 w-3.5" /> Scene {c.scene}
                  </a>
                ))}
              </div>
            </details>
